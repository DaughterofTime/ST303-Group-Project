...
sample_envelope_3 <- Y[U < (cos(Y)^{2*m-1}*exp(-nu*Y))/(cos(temp)^{2*m-1}*exp(-nu*temp))]
...
# Generate Y
n <- 10^5
Y <- alpha * tan(pi*runif(n) - pi/2) + lambda
```
...
temp_1 <- (Y - lambda)/alpha
temp_2 <- nu/(2*(1 - m))

sample_envelope_4 <- Y[U < ((1 + temp_1^2)^{-m} * exp(-nu * atan(temp_1)))/((1 + temp_2^2)^{1-m} * exp(-nu*atan(temp_2)))]
...
sample_envelope_5 <- Y[U < exp(-nu*atan((Y - lambda)/alpha) - nu*pi/2)]

plot(density(sample_envelope_5), type = 'l')
```

```{r}
# empirical rate of acceptance
length(sample_envelope_5)/n
```

```{r}
print(c(mean(sample_envelope_5), var(sample_envelope_5)))
check_mean_var(sample_envelope_5)
```