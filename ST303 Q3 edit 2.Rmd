...
Assuming no discounting factor, i.e. $r=0$ thus $e^{-rt}=1$, the pricing formula of the Bermuda call option is $E(max((Rti-K)^+, E(V_{i+1}|Fi)))$, where $V_{i+1}$ is the future value. Hence this is equivalent to find the maximum value of the payoffs all along the exercisable times, i.e. $max((R_{t_1}-K)^+,..., (R_{t_{10}}-K)^+)$
...
Let $X = max(max(R_t)-K,\ 0), \ for\ t=1,2,...,10$.
Set $\tilde{X} = max(R_t)-K$, then $Y = \tilde{X} - E(\tilde{X})$. Check: $E(Y) = 0$.
...
mean(Y) # E(Y)= -7.773459e-17 which is approximately equal to zero
```
...
(var(X)-var(theta_hat))/var(X)

cat(var(X), var(theta_hat), "Improved by 99.38%")
```
